...
Do not forget to check other shells: sh, ash, bsh, csh, ksh, zsh, pdksh, tcsh, bash
...
Static socat binaries can be found at [https://github.com/andrew-d/static-binaries](https://github.com/andrew-d/static-binaries/raw/master/binaries/linux/x86_64/socat).
...
Java reverse shells also work in Groovy.
...
To catch the shell, you need to listen on the desired port. `rlwrap` improves the shell and lets you use `[CTRL] + [L]`.
...
Oh My Zsh may break this trick; a simple `sh` is recommended.

> > The main issue here is that zsh does not handle the stty command the way bash or sh does. [...] `stty raw -echo; fg` [...] If you try to execute it as two separate commands, by the time you are prompted to run `fg`, your `-echo` command has already lost its effect.
...
Alternatively, use a `socat` binary to get a fully interactive TTY reverse shell.
...
The introduction of the pseudoconsole (ConPty) in Windows greatly improved how Windows handles terminals.

ConPtyShell uses the `CreatePseudoConsole()` function. This feature has been available since Windows 10 / Windows Server 2019 version 1809 (build 10.0.17763).
...
Reference: https://github.com/antonioCoco/ConPtyShell/blob/master/Invoke-ConPtyShell.ps1
